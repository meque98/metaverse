...
system_message =[{'role' : 'system' , 'content' : "You're going to role play with me in Japanese."}]
...
def setKey():
    ...
    Japanese.logger.warn("key : " + str(openai.api_key))
...
@Japanese.route('/makeChat') # chat에 emoji랑 summary 추가 / role 추가 / user에 role_list에 추가 및 징수
class JapaneseSimple(Resource):
    def post(self):
        Japanese.logger.info("/chat/makeChat")
        problem = request.json.get('problem')
        problem_collection = db['Japanese']
        chat_collection = db['JapaneseChat']
...
@Japanese.route('/askGPT')
class Japanesesimple(Resource):
    def post(self):
        chat_id = request.json.get('chat_id')
        text = request.json.get('text')
        Japanese.logger.info("/chat/askGPT "+ ", chat_id = "+chat_id)
        chat_collection = db['JapaneseChat']
